...
(function($) {
  ...
  Collapse.prototype = {
    ...
    close_: function(eq) {
      if(isFinite(eq)) return this.sections[eq].close_();
      $.each(this.sections, function() {
        this.close_();
      });
    }
  ...
  Section.prototype = {
    toggle : function() {
      if(this.isOpen) this.close_();
      else this.open();
    },
    close_: function(bypass) {
      this._changeState("close_", bypass);
    },
    ...
    _changeState: function(state, bypass) {
      ...
      _this.$summary.removeClass("open close_").addClass(state);
      _this.$details.attr("aria-hidden", state == "close_");
